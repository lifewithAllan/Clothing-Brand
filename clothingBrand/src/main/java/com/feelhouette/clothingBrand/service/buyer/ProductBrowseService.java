...
public class ProductBrowseService {
    ...
    public ProductResponseDTO getById(Long id) {
        Jersey j = jerseyRepo.findById(id)
    ...
    public List<ProductResponseDTO> search(String q) {
        ...
        return jerseyRepo.findAll().stream()
                .filter(j -> List.of(
                                ...
                                j.getSeason(),
                                j.getKitVersion(),
                                // join all league names into one string for searching
                                j.getLeagues() != null
                                        ? j.getLeagues().stream()
    ...
    private ProductResponseDTO map(Jersey j) {
        return new ProductResponseDTO(
                ...
                j.getSeason(),
                j.getKitVersion(),
                // return all league names as a list
                j.getLeagues() != null
                        ? j.getLeagues().stream().map(League::getLeagueName).toList()
                        : List.of(),
                j.getSizes(),
